...
public class ApiCoreRequests {
    ...
    @Step("Отправка POST-запроса на {url} с данными {data}")
    public Response makePostRequest(String url, Object data) {
        return RestAssured.given()
                .contentType(ContentType.JSON) // Устанавливаем тип контента
                .body(data) // Добавляем тело запроса
                .when()
                .post(url) // Отправляем POST-запрос
                .then()
                .extract()
                .response(); // Извлекаем ответ
    }

    @Step("Отправка POST-запроса для логина на {url} с данными {data}")
    public Response makePostRequestForLogin(String url, Object data) {
        return RestAssured.given()
                .contentType(ContentType.JSON) // Устанавливаем тип контента
                .body(data) // Добавляем тело запроса
                .when()
    ...
    public Response registerUser(Map<String, String> userData) {
        return makePostRequest("https://playground.learnqa.ru/api_dev/user/", userData);
    }
    @Step("Login user")
    public Response loginUser(Map<String, String> loginData) {
        return makePostRequestForLogin("https://playground.learnqa.ru/api_dev/user/login", loginData);
    }
